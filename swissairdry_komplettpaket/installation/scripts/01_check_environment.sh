...
check_cloud_py_api() {
    ...
    # Mit occ-Kommando prüfen, ob Cloud-Py-Api installiert ist
    APP_STATUS=$(docker exec -u www-data swissairdry_nextcloud php occ app:list | grep cloud_py_api)
    ...
    APP_ENABLED=$(docker exec -u www-data swissairdry_nextcloud php occ app:list | grep -A1 cloud_py_api | grep disabled)
    if [ ! -z "$APP_ENABLED" ]; then
